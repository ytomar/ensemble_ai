...
class QueryProcessor:
    ...
    def parse_query(self, query, query_type: QueryType, payload=None):
        ...
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "user", "content": self._create_prompt(query, query_type, payload=payload)}
            ]
        )
    ...
    def _create_prompt(query, query_type: QueryType, payload=None):
        ...
        if query_type == query_type.LOAD:
            return (f"Parse '`{query}`' into a structured output a valid json with 4 keys mentioned below:\n"
                    ...
                    f"Note: Source is {default_source} if not mentioned, Symbol is the ticker of the mentioned company on stock exchanges, "
                    f"Date formats are YYYY-MM-DD, START_DATE if not mentioned defaults to null, END_DATE if not mentioned defaults to null\n")
            ...
            return (f"Parse JSON output mentioned below enclosed in 3 backticks, return the real ticker matching the {query}\n"
                    f"```{json.dumps(connector.query_name(query))}```\n"
                    f"Note: You just have to return a `single` string symbol as output''. Do not write english as output\n"
                    )
...
if __name__=="__main__":
    ...
    load_command = query_processor.parse_query(load_query, 'load')
    print(load_command)

    transform_query = "Show closing prices for the last 5 days"
    transform_command = query_processor.parse_query(transform_query, 'transform')
